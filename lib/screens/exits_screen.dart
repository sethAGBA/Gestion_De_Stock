...
class _ExitsScreenState extends State<ExitsScreen> {
  ...
  void _showExitsDialog(BuildContext context, List<Map<String, dynamic>> exits, bool isDarkMode) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text('Détails des sorties de stock', style: Theme.of(context).textTheme.titleMedium),
        content: SizedBox(
          ...
          child: exits.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final typeKey = exit['type'] as String? ?? 'other';
        ...
        actions: [
          TextButton(
            ...
            child: Text('Fermer', style: Theme.of(context).textTheme.labelLarge),
          ),
        ],
      ),
    );
  ...
  Future<void> _exportToPdf() async {
    try {
      ...
      DateTime? filterEnd;
      if (_dateFilterMode == 'single' && _selectedDate != null) {
        filterStart = DateTime(_selectedDate!.year, _selectedDate!.month, _selectedDate!.day);
        filterEnd = filterStart;
      } else if (_dateFilterMode == 'range' && _selectedDateRange != null) {
      ...
      final items = exits.map((exit) {
        ...
        final quantite = (exit['quantite'] as num).toDouble();
        final valeurStock = quantite * (produit.prixVente ?? 0.0);
        return {
      ...
      final file = await PdfService.saveExitsReport(
        ...
        date: DateTime.now(),
        magasinAdresse: '',
        utilisateurNom: _currentUserName,
        items: items,
      ...
      final targetFile = await File(targetPath).create(recursive: true);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Rapport PDF exporté : $targetPath')),
        );
      }
    } catch (e) {
  ...
  Future<void> _exportToExcel() async {
    try {
      ...
      final labelDateFormat = DateFormat('dd/MM/yyyy', 'fr_FR');
      if (filterStart != null && filterEnd != null) {
        final filterLabel = filterStart.isAtSameMomentAs(filterEnd)
            ? labelDateFormat.format(filterStart)
            : '${labelDateFormat.format(filterStart)} - ${labelDateFormat.format(filterEnd)}';
        sheet.appendRow([
          excel.TextCellValue('Période'),
          excel.TextCellValue(filterLabel),
        ]);
        sheet.appendRow([]);
      }
      ...
      for (var i = 0; i < exits.length; i++) {
        ...
        final quantite = (exit['quantite'] as num).toDouble();
        final valeurStock = quantite * prixUnitaire;
        final date = DateTime.fromMillisecondsSinceEpoch(exit['date'] as int);
        sheet.appendRow([
          ...
          excel.TextCellValue(numberFormat.format(quantite)),
      ...
      final filePath = path.join(directoryPath, '$fileName.xlsx');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Sorties de stock', style: Theme.of(context).textTheme.titleLarge),
        elevation: 2,
      ...
      child: Row(
        children: [
          ..._ExitsScreenState._filters.entries.map((entry) => Padding(
                ...
                child: FilterChip(
                  label: Text(entry.value, style: Theme.of(context).textTheme.bodyMedium),
          ...
          ElevatedButton.icon(
            ...
            label: Text('Choisir une date', style: Theme.of(context).textTheme.bodyMedium),
          ...
          ElevatedButton.icon(
            ...
            label: Text('Choisir une période', style: Theme.of(context).textTheme.bodyMedium),
            style: ElevatedButton.styleFrom(
              backgroundColor: isDarkMode ? Colors.grey.shade700 : Colors.grey.shade200,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
            ),
            ...
            Padding(
              ...
              child: Chip(
                label: Text(
                  ...
                  style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: theme.primaryColor),
                ),
    ...
    return FutureBuilder<List<Produit>>(
      ...
      builder: (context, produitSnapshot) {
        ...
        if (produitSnapshot.hasError) {
          ...
          return Center(child: Text('Erreur : ${produitSnapshot.error}', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Colors.red)));
        }
        ...
        return FutureBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (context, exitSnapshot) {
            ...
            if (exitSnapshot.hasError) {
              ...
              return Center(child: Text('Erreur : ${exitSnapshot.error}', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Colors.red)));
            }
                            ...
                            return DataRow(
                              ...
                              cells: [
                                ...
                                DataCell(
                                  SizedBox(
                                    width: 150,
                                    child: Text(produit.categorie ?? 'N/A', style: Theme.of(context).textTheme.bodyMedium),
                                  ),
                                ),
                                DataCell(
                                  SizedBox(
                                    width: 100,
                                    child: Text(produit.unite ?? 'N/A', style: Theme.of(context).textTheme.bodyMedium),
                                  ),
                                ),
                                DataCell(
                                  SizedBox(

...
class _ProductDialogState extends State<ProductDialog> {
  ...
  double _prixConditionnement = 0.0;
  ...
  late final TextEditingController _codeBarresController;
  ...
  void initState() {
    ...
    if (widget.produit != null) {
      ...
      _prixConditionnement = widget.produit!.prixConditionnement;
    ...
    _skuController = TextEditingController(text: _sku ?? '');
    _codeBarresController = TextEditingController(text: _codeBarres ?? '');
  }
  ...
  String _buildSkuFromName() {
    final base = _nom.isNotEmpty ? _nom : 'PRD';
    final cleaned = base.toUpperCase().replaceAll(RegExp(r'[^A-Z0-9]'), '');
    final prefix = cleaned.isEmpty ? 'PRD' : cleaned.substring(0, cleaned.length.clamp(0, 6));
  ...
  String _buildBarcode() {
    final ts = DateTime.now().millisecondsSinceEpoch.toString();
    final numeric = ts.padLeft(12, '0').substring(ts.length > 12 ? ts.length - 12 : 0);
    return numeric;
  }
  ...
  Future<void> _addProduct() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (_prixVenteGros > 0 && _seuilGros <= 0) {
        _seuilGros = 1; // Empêche les ventes en gros illimitées par défaut
      }
      if ((_conditionnementLabel == null || _conditionnementLabel!.isEmpty) || _conditionnementQuantite <= 0) {
        _conditionnementLabel = null;
        _conditionnementQuantite = 0;
        _prixConditionnement = 0;
      }
      ...
      final produit = Produit(
        ...
        prixConditionnement: _prixConditionnement,
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          suffixIcon: IconButton(
                            tooltip: 'Générer un SKU',
                            icon: const Icon(Icons.confirmation_number_outlined),
                            onPressed: () {
                              final generated = _buildSkuFromName();
                              setState(() {
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          border: const OutlineInputBorder(),
                          suffixIcon: IconButton(
                            tooltip: 'Générer un code-barres',
                            icon: const Icon(Icons.qr_code_2_outlined),
                            onPressed: () {
                              final generated = _buildBarcode();
                              setState(() {
                                _codeBarresController.text = generated;
                                _codeBarres = generated;
                              });
                            },
                          ),
                        ),
                      ...
                      const SizedBox(height: 16),
                      Text('Conditionnement (optionnel)', style: TextStyle(fontWeight: FontWeight.w600, color: Theme.of(context).colorScheme.primary)),
                      const SizedBox(height: 8),
                      Row(
                        children: [
                          Expanded(
                            child: TextFormField(
                              initialValue: _conditionnementLabel ?? '',
                              decoration: const InputDecoration(labelText: 'Libellé (ex: Paquet, Carton)', border: OutlineInputBorder()),
                              onSaved: (value) => _conditionnementLabel = (value ?? '').trim().isEmpty ? null : value!.trim(),
                            ),
                          ),
                          const SizedBox(width: 12),
                          Expanded(
                            child: TextFormField(
                              initialValue: _conditionnementQuantite == 0 ? '' : _conditionnementQuantite.toString(),
                              decoration: const InputDecoration(labelText: 'Qté par conditionnement', border: OutlineInputBorder()),
                              keyboardType: const TextInputType.numberWithOptions(decimal: true),
                              inputFormatters: [
                                FilteringTextInputFormatter.allow(RegExp(r'[0-9.,\\s-]')),
                              ],
                              ...
                              onSaved: (value) => _conditionnementQuantite = _parseDoubleLocale(value) ?? 0.0,
                            ),
                          ),
                        ],
                      ),
                      ...
                      TextFormField(
                        initialValue: _prixConditionnement == 0 ? '' : _prixConditionnement.toString(),
                        decoration: const InputDecoration(labelText: 'Prix du conditionnement', border: OutlineInputBorder()),
                        keyboardType: const TextInputType.numberWithOptions(decimal: true),
                        inputFormatters: [
                          FilteringTextInputFormatter.allow(RegExp(r'[0-9.,\\s-]')),
                        ],
                        validator: (value) {
                          if (value != null && value.isNotEmpty) {
                            final num = _parseDoubleLocale(value);
                            if (num == null) return 'Nombre invalide';
                            if (num < 0) return 'Ne peut pas être négatif';
                          }
                          return null;
                        },
                        onSaved: (value) => _prixConditionnement = _parseDoubleLocale(value) ?? 0.0,
                      ),
                      const SizedBox(height: 16),
                      Row(
                        children: [
